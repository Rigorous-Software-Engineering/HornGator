; ./prepared/rust-horn/./trees-2-inc-all-safe_000.smt2
(set-logic HORN)
...
(assert
  (forall ( (A ~Mut<%Tree>) (B ~Mut<%Tree>) ) 
    (=>
      (and
        (and (= A (~mut<%Tree> %Tree-1 (~ret<%Tree> B))) (= (~ret<%Tree> B) %Tree-1))
      )
      (%inc.0 A)
    )
  )
