; ./prepared/rust-horn/./lists-4-inc-some2-safe_000.smt2
(set-logic HORN)

(declare-datatypes ((~Mut<Int> 0)) (((~mut<Int>  (~cur<Int> Int) (~ret<Int> Int)))))
(declare-datatypes ((~Mut<%List> 0) (%List 0)) (((~mut<%List>  (~cur<%List> %List) (~ret<%List> %List)))
                                                ((%List-0  (%List-0.0 Int) (%List-0.1 %List)) (%List-1 ))))
  ...
  (forall ( (A ~Mut<%List>) (B ~Mut<%List>) (C Bool) (D ~Mut<Int>) (E ~Mut<%List>) (F ~Mut<Int>) (G Bool) (H Int) (I ~Tup<~Mut<Int>-~Mut<%List>>) (J ~Tup<~Mut<Int>-~Mut<%List>>) (K Int) (L %List) (M %List) (N %List) (O %List) ) 
    (=>
      (and
        ...
        (%take_some_rest A J)
        (%sum M K)
        (let ((a!1 (~mut<Int> (+ 1 (~cur<Int> (~at0/<~Mut<Int>-~Mut<%List>> I)))
                      (~ret<Int> (~at0/<~Mut<Int>-~Mut<%List>> I))))
      (a!2 (~mut<Int> (+ 1 (~cur<Int> (~at0/<~Mut<Int>-~Mut<%List>> J)))
                      (~ret<Int> (~at0/<~Mut<Int>-~Mut<%List>> J))))
      (a!3 (= A (~mut<%List> (~cur<%List> (~at1/<~Mut<Int>-~Mut<%List>> I)) O)))
      (a!4 (= E (~mut<%List> O (~ret<%List> (~at1/<~Mut<Int>-~Mut<%List>> I)))))
      (a!5 (not (= (= K (+ 2 H)) C))))
  (and (= (~ret<%List> (~at1/<~Mut<Int>-~Mut<%List>> J))
